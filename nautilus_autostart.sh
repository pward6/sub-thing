...
echo "[autostart] waiting for /nucleus_node/ins_packets + /mavros/state (up to 150s)"
END=$(( $(date +%s) + 150 ))
until ros2 topic list 2>/dev/null | grep -q "/nucleus_node/ins_packets" \
   && ros2 topic list 2>/dev/null | grep -q "/mavros/state"; do
  if [ "$(date +%s)" -gt "$END" ]; then
    echo "[autostart] stack did not come up within 150s -- aborting, NOT arming."
    kill "$STACK_PID" 2>/dev/null
    exit 1
  fi
  sleep 2
done
echo "[autostart] stack is up. Launching hard-code mission."
